...
export const createLoadStockActionThunk = outletId => dispatch => {
    dispatch(createWaitAction("Please wait while refreshing data..."));
    axios.get(apiUrl + "?outletId=" + outletId)
...
export const createAddItemActionThunk = item => dispatch => {
    ...
    axios.post(apiUrl, item)
        .then(resp => createLoadStockActionThunk(item.outletId)(dispatch))
...
export const createDeleteItemActionThunk = (id,outletId) => dispatch => {
    dispatch(createWaitAction("Please wait while removing a record..."));
    axios.delete(apiUrl+"/"+id)
        .then(resp => createLoadStockActionThunk(outletId)(dispatch))
